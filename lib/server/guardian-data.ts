...
function docsWithIds(docs: QueryDocumentSnapshot[]): LooseDoc[] { return docs.map((doc) => ({ id: doc.id, ...(doc.data() as Record<string, unknown>) })); }
...
export async function loadGuardianBundle(studentId: string) {
  ...
  const activeTerm = activeTermDoc ? ({ id: activeTermDoc.id, ...(activeTermDoc.data() as Record<string, unknown>) } as LooseDoc) : null;
  ...
  const [subjects, weeklyPlans, skills, assessmentsSnap, resourcesSnap, values, starsSnap, spellingPractices, messagesSnap, followUpSnap] = await Promise.all([
    byTerm("subjects"), byTerm("weeklyPlans"), byTerm("skills"),
  ...
  const guardianSubjects = subjects.filter((subject) => subject.enabled !== false).sort((a,b) => Number(a.order ?? 0)-Number(b.order ?? 0));
  const guardianWeeklyPlans = weeklyPlans.sort((a,b) => Number(a.week ?? 0)-Number(b.week ?? 0));
  const assessments = docsWithIds(assessmentsSnap.docs).filter((assessment) => allowedSkillIds.has(String(assessment.skillId ?? "")));
  const resources = docsWithIds(resourcesSnap.docs).filter((resource) => resource.publishedToGuardian === true && (!termId || resource.termId === termId)).map(({ answerGuide: _answerGuide, ...resource }) => resource).sort((a,b) => String(b.createdAt ?? "").localeCompare(String(a.createdAt ?? "")));
  ...
  const messages = docsWithIds(messagesSnap.docs).sort((a,b) => String(a.createdAt ?? "").localeCompare(String(b.createdAt ?? ""))).slice(-50);
