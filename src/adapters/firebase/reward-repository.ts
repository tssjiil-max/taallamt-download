...
export class FirestoreRewardRepository implements RewardRepository{
 ...
 async addStars(studentId:string,month:string,amount:number){if(amount<=0)return;const db=firebaseDb(),ref=doc(db,paths.rewards(this.workspaceId),this.id(studentId,month));await runTransaction(db,async tx=>{const snap=await tx.get(ref);const current=snap.exists()?Number(snap.data().stars||0):0;tx.set(ref,{studentId,month,stars:Math.min(30,current+amount)},{merge:true})})}
}
