...
function Teacher(){
 ...
 return <main className="screen teacher" dir="rtl">
  ...
  <section className="nowCard">
    <div className="nowHeading"><h2><UiIcon name="clock" size={27}/>حصتي الآن</h2><div className="timePills"><span>10:00 - 10:45</span><b>{CURRENT_LESSON.period}</b></div></div>
    <div className="nowBody">
      <button className="startLesson" onClick={()=>go('/teacher/lesson')}><UiIcon name="play" size={23}/>ابدأ الحصة</button>
      <div className="lessonDetails"><h3><SubjectIcon kind="islamic"/>{CURRENT_LESSON.subjectTitle}</h3><p>الوحدة: {CURRENT_LESSON.unit}</p><p>الدرس: {CURRENT_LESSON.lesson}</p><p><b>المهارات:</b> {CURRENT_LESSON.skills.join(' – ')}</p></div>
    </div>
  </section>
...
function TeacherNav(){const p=location.pathname;return <nav className="bottomNav teacherNav"><button className={p==='/teacher'||p==='/'?'active':''} onClick={()=>go('/teacher')}><UiIcon name="home" size={28}/><b>الرئيسية</b></button><button className={p==='/teacher/students'?'active':''} onClick={()=>go('/teacher/students')}><UiIcon name="people" size={29}/><b>الطلاب</b></button><button className="mascotNav" type="button"><span><Mascot/></span><b>شكابمبو</b></button><button className={p==='/teacher/library'?'active':''} onClick={()=>go('/teacher/library')}><UiIcon name="books" size={29}/><b>الكتب</b></button><button className={p==='/teacher/settings'?'active':''} onClick={()=>go('/teacher/settings')}><UiIcon name="more" size={29}/><b>المزيد</b></button></nav>}
