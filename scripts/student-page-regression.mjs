...
const checks=[
  ...
  ['locked cards keep their order and exact headings',studentEvaluation.includes('taskPanel')&&studentEvaluation.includes('evaluationPanel')&&!studentEvaluation.includes('insertBefore(taskPanel')&&studentEvaluation.includes("heading.textContent='تقييمي اليوم'")],
  ['teacher evaluation is limited to today in Riyadh time',studentEvaluation.includes("timeZone:'Asia/Riyadh'")&&studentEvaluation.includes('assessmentIsToday')&&studentEvaluation.includes('todayAcademicMap')],
  ['student evaluation displays only the approved academic labels',studentEvaluation.includes('أتقن')&&studentEvaluation.includes('يحتاج تدريب')&&!studentEvaluation.includes('لم يتقن')],
  ['locked student evaluation includes behavior but keeps values out of this card',studentEvaluation.includes('السلوك العام')&&!studentEvaluation.includes('القيم')],
  ...
  ['isolated evaluation api accepts only mastered and needs practice',evaluationApi.includes("['mastered','needs_practice']")&&!evaluationApi.includes("'not_mastered'" )],
  ...
  ['teacher academic assessment does not add a third academic state',!teacherEvaluation.includes("dataset.value='not_mastered'")&&!teacherEvaluation.includes('لم يتقن')],
