...
const checks=[
  ...
  ['student state exposes curriculum targets',studentState.includes('curriculum:context.curriculum')||studentState.includes('curriculum:curriculumRows')],
  ['student state exposes published weekly plan',studentState.includes('weeklyPlan:context.weeklyPlan')||studentState.includes('weeklyPlan:weeklyPlanRows')],
  ['student state exposes lightweight read views',['profile','reward','assessment_meta','weekly_values','evaluation','homework','activity'].every(view=>studentState.includes(`view==='${view}'`))],
  ['student state caches shared weekly context',studentState.includes('WEEKLY_CONTEXT_TTL_MS')&&studentState.includes('weeklyContextCache')],
  ['student interactions avoid background polling',!interactions.includes('setInterval(applyTaskStatus')&&interactions.includes('__taallamtStudentStateViewFetch')],
