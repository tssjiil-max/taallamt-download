...
const previewDateFromRequest=(req)=>{
...
export default async function handler(req,res){
  try{const action=String(req.query?.action||req.body?.action||'preview');if(action==='preview'){const previewDate=previewDateFromRequest(req);if(!previewDate)return res.status(400).json({ok:false,error:'DATE_INVALID'});return res.status(200).json(await previewAutomation(previewDate))}if(req.method!=='POST')return res.status(405).json({ok:false,error:'METHOD_NOT_ALLOWED'});if(action==='assistant'){try{return res.status(200).json({ok:true,...await runLessonAssistant(req.body||{})})}catch(error){const message=error instanceof Error?error.message:String(error);if(['MODE_INVALID','QUESTION_REQUIRED','SUBJECT_INVALID','LESSON_CONTEXT_REQUIRED'].includes(message))return res.status(400).json({ok:false,error:message});console.error('SHAKABAMBO_ASSISTANT_ERROR',{name:error?.name||'Error',message,statusCode:error?.statusCode||error?.status||null,hasOidc:Boolean(process.env.VERCEL_OIDC_TOKEN),hasGatewayKey:Boolean(process.env.AI_GATEWAY_API_KEY)});return res.status(503).json({ok:false,error:'ASSISTANT_UNAVAILABLE',message:'تعذر تشغيل شكابمبو الآن. الحصة نفسها ما زالت جاهزة ويمكن متابعة الخطة بدون المساعد.'})}}if(action==='seed'){const curriculum=await seedCurriculum();const weekly=await publishWeeklyPlan(weekNumberForDate());return res.status(200).json({ok:true,curriculum,weekly})}if(action==='weekly')return res.status(200).json({ok:true,...await publishWeeklyPlan(Number(req.body?.week)||weekNumberForDate(new Date(Date.now()+86400000))) });if(action==='daily')return res.status(200).json({ok:true,...await publishDailyHomework()});return res.status(400).json({ok:false,error:'ACTION_INVALID'})}catch(error){return res.status(500).json({ok:false,error:error instanceof Error?error.message:String(error)})}
}
