...
export default function TeacherHome(){
 ...
 return <main className="ta-app ta-teacher-home">
  ...
  <Link className="ta-quick-assessment" href="/teacher/assessment"><Image src="/teacher-icons/assessment.svg" alt="" width={82} height={82}/><div><small>أسرع إجراء أثناء الحصة</small><h2>التقييم السريع</h2><p>حدد «أتقن» للجميع ثم عدّل من يحتاج تدريبًا</p></div><span>‹</span></Link>
  <section className="ta-action-grid"><Link href="/teacher/students"><Image src="/teacher-icons/students.svg" alt="" width={58} height={58}/><div><b>الطلاب</b><span>الملفات والمتابعة</span></div></Link><Link href="/teacher/values"><Image src="/guardian-icons/trophy.svg" alt="" width={58} height={58}/><div><b>السلوك والتحفيز</b><span>الفصل · المدرسة · المنزل</span></div></Link><Link href="/teacher/library"><Image src="/guardian-icons/library.svg" alt="" width={58} height={58}/><div><b>المكتبة</b><span>ملفات خاصة وعامة</span></div></Link><Link href="/teacher/announcements"><Image src="/guardian-icons/message.svg" alt="" width={58} height={58}/><div><b>التواصل</b><span>الإعلانات وطلبات التواصل</span></div></Link></section>
  <section className="ta-card ta-recent-panel"><div className="ta-section-title"><div><small>آخر تحديثاتك المحفوظة</small><h2>آخر ما سجل المعلم</h2></div><Link href="/teacher/reports">كل النتائج</Link></div><div className="ta-recent-list">{recentAssessments.length?recentAssessments.map(item=>{const student=store.students.find(studentItem=>studentItem.id===item.studentId),skill=store.skills.find(skillItem=>skillItem.id===item.skillId);return <Link href={`/teacher/students/${item.studentId}`} key={item.id}><span className={`ta-result-dot ${item.level}`}>✓</span><div><b>{student?.name??"طالب"}</b><small>{skill?.title??"تقييم مهارة"}</small></div><strong>{levelLabel[item.level]}</strong><time>{shortDate(item.assessedAt)}</time></Link>}):<div className="ta-empty-line">لم تُسجّل تقييمات بعد هذا الفصل.</div>}</div></section>
