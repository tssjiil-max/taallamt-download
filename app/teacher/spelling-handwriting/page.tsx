  ...
  return (
    <main className="shell">
      ...
      <section className="section" id="week-details">
        {practice ? <div className="card"><div className="section-head"><h2>الأسبوع {practice.week} — {practice.unitName}</h2><span className="badge">من {practice.scoreTotal}</span></div><div className="kv"><span>مهارة الإملاء</span><b>{practice.skill}</b></div><h3 className="section">معايير الخط</h3><ul>{practice.handwritingChecklist.map((item) => <li key={item} style={{ marginBottom: 9 }}>{item}</li>)}</ul><div className="notice section">نمط التدريب المعتمد من الكتيب: تتبع النموذج، ثم النسخ مع النظر، ثم الكتابة المستقلة مع المحافظة على السطور الأربعة.</div></div> : <div className="notice">لا يوجد تدريب مسجل لهذا الأسبوع.</div>}
      </section>

      <section className="section"><div className="section-head"><h2>خريطة الفصل</h2><span className="pill">اختر أسبوعًا لفتح التفاصيل</span></div><div className="list">{store.spellingPractices.filter((item) => item.termId === store.activeTermId).map((item) => <button type="button" className="row" key={item.id} onClick={() => openWeek(item.week)}><div className="row-main"><div className="avatar">{item.week}</div><div><h4>{item.skill}</h4><small>{item.unitName}</small></div></div><span className="badge">{item.week === week ? "مفتوح" : "فتح"}</span></button>)}</div></section>
