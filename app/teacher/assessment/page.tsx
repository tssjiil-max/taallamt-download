...
const levels: MasteryLevel[] = ["mastered", "needs_training"],
  labels: Record<MasteryLevel, string> = {
    mastered: "أتقن",
    partial: "يحتاج تدريب",
...
function latest(
  ...
  const found = a
    .filter((x) => x.studentId === studentId && x.skillId === skillId)
    .at(-1)?.level;
  return found === "partial" ? "needs_training" : found;
}
export default function AssessmentPage() {
  const store = useTaallamt(),
    term = store.terms.find((t) => t.active) ?? store.terms[0],
    subjects = store.subjects
      .filter((s) => s.enabled && s.termId === term?.id)
      .sort((a, b) => a.order - b.order);
  const [subjectId, setSubjectId] = useState(subjects[0]?.id ?? "quran"),
    [week, setWeek] = useState(academicWeek()),
  ...
  const skills = store.skills.filter(
    ...
    skill = skills.find((s) => s.id === skillId) ?? skills[0],
    students = store.students.filter((s) => s.active);
  const stats = useMemo(() => {
    if (!skill) return { mastered: 0, needs: 0, pending: students.length };
    const values = students.map((s) =>
      latest(store.assessments, s.id, skill.id),
    );
    return {
      mastered: values.filter((x) => x === "mastered").length,
      needs: values.filter((x) => x === "needs_training").length,
  ...
  async function applyToAll() {
    if (
      !skill ||
      !confirm(
        "تسجيل «أتقن» لجميع الطلاب؟ بعد الحفظ عدّل فقط من يحتاج تدريبًا.",
      )
    )
      return;
    setSaving("all");
    ...
    <main className="shell inner-shell">
      <header className="subject-hero">
        ...
        <div>
          ...
          <h1>التقييم السريع</h1>
          <p>مهارتان واضحتان: أتقن أو يحتاج تدريبًا</p>
        </div>
        ...
        <div className="picker-block">
          ...
          {skills.length ? (
            <div className="assessment-skill-grid">
              {skills.map((s) => (
                <button
                  className={skill?.id === s.id ? "active" : ""}
                  key={s.id}
            ...
            <div className="empty-state">
              لا توجد مهارة لهذه المادة في الأسبوع {week}.
            </div>
          )}
        </div>
      </section>
          ...
          <div className="clean-eval-list">
            {students.map((student, index) => {
              const value = latest(store.assessments, student.id, skill.id),
                rowSaving = saving === `${student.id}:${skill.id}`;
              return (
