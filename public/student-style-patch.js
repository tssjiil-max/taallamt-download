...
const SHAKABUMBO_PROFILE=`${RAW}/teacher-icons/shakabumbo-logo.svg`;
const SHAKABUMBO_TROPHY=`${RAW}/student-icons/achievement-trophy.svg`;
...
ensureImg(avatarWrap,'.studentProfileAvatar','studentProfileAvatar',uploadedPhoto||SHAKABUMBO_PROFILE,'صورة الطالب');
const starToday=student.querySelector('.starToday');
if(starToday){starToday.querySelector('.starIcon')?.remove();ensureImg(starToday,'.studentStarMascot','studentStarMascot',SHAKABUMBO_TROPHY,'شكابمبو ونجمة اليوم')}
const rewardMascot=student.querySelector('.rewardMascot');
if(rewardMascot){ensureImg(rewardMascot,'img','rewardMascotImage',SHAKABUMBO_TROPHY,'شكابمبو مع الكأس')}
const navMascot=student.querySelector('.studentNav .mascotNav>span');
if(navMascot){const old=navMascot.querySelector('.mascot');if(old&&old.tagName==='IMG'){old.src=SHAKABUMBO_PROFILE;old.alt='شكابمبو'}else{ensureImg(navMascot,'.studentNavMascot','studentNavMascot',SHAKABUMBO_PROFILE,'شكابمبو')}}
