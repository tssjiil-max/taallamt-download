...
async function openStudentPanel(kind,subject){
  if(kind==='photo'){const state=await loadState();const body=document.createElement('div');body.className='studentPhotoPanel';const img=document.createElement('img');img.src=state?.profile?.photoDataUrl||readStudent().photoDataUrl||SHAKABUMBO_PROFILE;const input=document.createElement('input');input.type='file';input.accept='image/*';input.addEventListener('change',()=>savePhoto(input.files?.[0]));body.append(img,input);panel('صورتي',body);return}
