(()=>{
  ...
  async function getPreview(force=false){
    if(!force&&previewCache&&Date.now()-previewAt<60000)return previewCache;
    if(previewBusy)return previewBusy;
    previewBusy=api('/api/learning-automation?action=preview').then(data=>{previewCache=data;previewAt=Date.now();return data}).finally(()=>{previewBusy=null});
    return previewBusy;
  }
  async function hydrateWeekly(force=false){
    try{
      const data=await getPreview(force),content=data.content||{};
      ...
      panel.querySelectorAll('.scheduleItem').forEach(row=>{
        ...
        const sub=row.querySelector('.scheduleText span');if(sub){const parts=[item.unit||item.surah,item.lesson||item.weekly,item.skill].filter(Boolean);sub.textContent=parts.join(' · ')}row.dataset.liveWeek=String(data.week||'');
      });
  ...
  function libraryError(body){body.innerHTML='<div style="padding:18px;text-align:center;color:#9c4b4b"><p style="margin:0 0 10px">تعذر تحميل المكتبة الآن.</p><button type="button" class="studentLibraryRetry" style="border:1px solid #b9dcf4;background:#eef7ff;color:#176cc5;border-radius:10px;padding:8px 12px;font-weight:800">إعادة المحاولة</button></div>';body.querySelector('.studentLibraryRetry')?.addEventListener('click',()=>{const modal=body.closest('.studentPatchModal');if(modal){delete modal.dataset.smartStudentLibrary;void hydrateStudentLibrary(modal)}})}
  async function hydrateStudentLibrary(modal){
    ...
    try{const sid=studentId();if(!sid)throw new Error('STUDENT_REQUIRED');const data=await api(`/api/library-files?role=student&studentId=${encodeURIComponent(sid)}`),files=data.files||[];body.className='studentLibraryPanel';body.innerHTML=files.length?files.map(fileCard).join(''):'<p style="padding:18px;text-align:center;color:#7890a2">لا توجد ملفات عامة أو خاصة بك حتى الآن.</p>'}catch{libraryError(body)}
  }
  function watchModals(){document.querySelectorAll('.studentPatchModal').forEach(hydrateStudentLibrary)}
  void hydrateWeekly();
  window.addEventListener('focus',()=>void hydrateWeekly(false));
  document.addEventListener('visibilitychange',()=>{if(!document.hidden)void hydrateWeekly(false)});
  new MutationObserver(()=>requestAnimationFrame(watchModals)).observe(document.getElementById('root')||document.body,{childList:true,subtree:true});
})();